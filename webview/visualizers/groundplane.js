export function redrawGroundPlane(nodesMin, nodesMax, selectedNodeIndices, pointsCache, jbeamData, currentPartName, nodeCounter) {
  ...
  const defaultfont = 'bold 60px "Cascadia Code", monospace'

  let items = [
    { type: 'text', position: new THREE.Vector3(0, 0, 0), font: 'bold 50px "Cascadia Code", monospace', color: '#444444', text: '(0,0,0)', textAlign: 'left', textBaseline: 'bottom' },
  ...
  if(nodesMin && nodesMax) {
    ...
    if(leftLength > 0.1) {
      items.push({ type: 'arrow', start: leftStart, end: leftEnd, color: 'rgba(0.3, 0.3, 0.3, 0.3)', width: labelSize / 10, label: leftLength + 'm', font: `bold ${labelSize}px "Cascadia Code", monospace` })
      // TODO:
    ...
    if(topLength > 0.1) {
      items.push({ type: 'arrow', start: topStart, end: topEnd, color: 'rgba(0.3, 0.3, 0.3, 0.3)', width: labelSize / 10, label: topLength + 'm', font: `bold ${labelSize}px "Cascadia Code", monospace`})
    }
  }
  ...
  if(selectedNodeIndices) {
    if(selectedNodeIndices.length == 1) {
      ...
      if(node) {
        ...
        items.push({ type: 'circle', position: node.rpos3d, size: 8, color: '#448844'})
        items.push({ type: 'text', position: node.pos3d, font: 'bold 40px "Cascadia Code", monospace', color: '#448844', text: node.name || "", textBaseline: 'bottom'})
        items.push({ type: 'line3d', pos1: new THREE.Vector3(node.rpos3d.x, node.rpos3d.y, 0), pos2: new THREE.Vector3(node.rpos3d.x, node.rpos3d.y, node.rpos3d.z), color: '#aaaaaa', dashSize:0.02, gapSize:0.01})
      }
      ...
      if(nodeA && nodeB) {
        ...
        items.push({ type: 'text', position: nodeA.pos3d, font: 'bold 40px "Cascadia Code", monospace', color: '#448844', text: nodeA.name || "", textAlign: 'right'})
        items.push({ type: 'text', position: nodeB.pos3d, font: 'bold 40px "Cascadia Code", monospace', color: '#448844', text: nodeB.name || "", textAlign: 'left'})
      }
    }
  }
  ...
  if(jbeamData) {
    if(currentPartName) {
      ...
      if(part) {
        ...
        items.push({ type: 'text', position: new THREE.Vector3(freeBoxText.x, 0, freeBoxText.z), font: 'bold 120px "Cascadia Code", monospace', color: '#aaaaaa', text: txt, textAlign: 'left', textBaseline: 'top'})
        txt = `${nodeCounter} nodes`
        if(nodeCounter == 1) txt = `${nodeCounter} node`
        else if(nodeCounter == 0) txt = `no nodes :(`

        items.push({ type: 'text', position: new THREE.Vector3(freeBoxText.x, 0, freeBoxText.z + 0.4), font: 'bold 60px "Cascadia Code", monospace', color: '#aaaaaa', text: txt, textAlign: 'left', textBaseline: 'top'})
      }
    } else {
      ...
      for (let partName in jbeamData) {
        items.push({ type: 'text', position: new THREE.Vector3(freeBoxText.x + 0.1, 0, freeBoxText.z + 0.3 + rowCounter * 0.2), font: 'bold 60px "Cascadia Code", monospace', color: '#aaaaaa', text: partName, textAlign: 'left', textBaseline: 'top'})
        rowCounter++
      }
    }
  }
